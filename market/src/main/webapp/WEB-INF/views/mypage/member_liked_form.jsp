				...
				<ul class="pagination">
					<c:if test="${page.startPage > page.pagePerBlk }">
						<li class="page-item"><a class="page-link"
							href="member_liked_form?page=${page.startPage-1}">Previous</a></li>
					</c:if>
					<c:forEach begin="${page.startPage}" end="${page.endPage}"
						var="pageNum">
						<li
							class="page-item <c:if test="${page.currentPage==pageNum}">active</c:if>"><a
							class="page-link" href="member_liked_form?page=${pageNum}">${pageNum}</a></li>
					</c:forEach>
					<c:if test="${page.endPage < page.totalPage}">
						<li class="page-item"><a class="page-link"
							href="member_liked_form?page=${page.startPage-1}">Next</a></li>
					</c:if>
				</ul>
			</div>
	</div>
